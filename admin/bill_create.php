<?php
	...
	if (isset($_GET['client_id'])) {
        ...
        $sql = "SELECT username FROM vartotojai WHERE id=$client_id";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    	...
    	$saskaitos_insert = "INSERT INTO saskaitos (user_id, total_price, date, series_number, user_created, bank_purpose, bank_number, bank_title) VALUES ('$client_id', '$total_price', '$now', '$series_number', '$issuer_id', '$bank_purpose', '$bank_number', '$bank_title')";

		if(mysqli_query($link, $saskaitos_insert)){
		    echo "Saskaita prideta";
		} else{
		    echo "Kazkas negerai. " . mysqli_error($link);
		}

		$saskaitos_id = $link->insert_id;
...
<html>
	...
	<body class="loggedin">
		...
		<form method='POST'>
		<div class="content">
			<h2>Israsyti saskaita klientui <b><?php echo $username; ?></b></h2>
				<div>
					<h2>Klientas</h2>
					<br>
					<?php
						echo '
							<select class="form-control" name="client" id="client" onchange="clientChange()">
				...
				<div id="saskaita">
					...
					<table>
						...
						<tr>
							...
							<td><input required class="form-control" name="series_number" type="text" maxlength="12" placeholder="Serijos numeris"></td>
						</tr>
					</table>
				</div>
				<div id="banko-saskaita">
					...
					<table>
						<tr>
							...
							<td><input class="form-control" type="text" name="bank_number" placeholder="LTXX XXXX XXXX XXXX XXXX" required ></td>
						</tr>
						<tr>
							<td>Moketojo pavadinimas:</td>
							<td><input required class="form-control" name="bank_title" type="text" placeholder="Moketojo pavadinimas"></td>
						</tr>
						<tr>
							<td>Mokejimo paskirtis:</td>
							<td><input required class="form-control" name="bank_purpose" type="text"placeholder="Mokejimo paskirtis"></td>
						</tr>
					</table>
					<div class="float-right"><button type="submit" class="btn btn-danger mb-2 btn-sm">Israsyti saskaita</button></div>
					<br>
				</div>
			</div>
		</form>
